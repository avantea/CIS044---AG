...
class UnsortedListPQ<K extends Comparable<K>, V> implements PriorityQueue<K, V> {
    ...
    public MyEntry<K, V> removeMin() {
        ...
        for (int i = 1; i < list.size(); i++) {
            MyEntry<K, V> current = list.get(i);
            if (current.compareTo(minEntry) < 0) {
                minEntry = current;
                minIndex = i;
            }
        }
...
class SortedListPQ<K extends Comparable<K>, V> implements PriorityQueue<K, V> {
    ...
    public void insert(K key, V value) {
        ...
        // If the list is empty, just add the new entry.
        if (list.isEmpty()) {
            list.add(newEntry);
            return;
        }

        // Walk through the list until we find an element greater than newEntry
        int index = 0;
        while (index < list.size() && list.get(index).compareTo(newEntry) <= 0) {
            index++;
        }

        // Insert at the found index (which might be at the end).
        list.add(index, newEntry);

        // Default insert at end if it's the largest
        // (Handled by the logic above when index == list.size())
